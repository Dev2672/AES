...
// ye wala code almost copy hi hai
'use strict';
 ...
 *
 * {number[]}   input = 16-byte (128-bit) input state array.
 * {number[][]} w = Key schedule as 2D byte-array (Nr+1 x Nb bytes).
 * {number[]}   Encrypted output state array.
 */
 ...
 *
 * {number[]}   key = Cipher key as 16/24/32-byte array.
 * {number[][]} Expanded key schedule as 2D byte-array (Nr+1 x Nb bytes).
 */
Aes.keyExpansion = function(key) {
    var Nb = 4;   // blocksize 
    var Nk = key.length/4; // key length= 4/6/8 for 128/192/256-bit keys
...
// Iske niche ka Comlete Copy hai
